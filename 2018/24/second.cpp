...
bool operator==(const Army& a, const Army& b) {
    return a.units == b.units && a.unitHP == b.unitHP && a.AD == b.AD && a.initiative == b.initiative;
}
...
int chooseTarget(Army attacker, std::vector<Army>& opp, const std::vector<int>& chosen) {
    ...
    for (int i = 0; i < opp.size(); ++i) {
        ...
        if (damage > bestDamage) {
            ...
            bestDamage = damage;
            bestArmy = opp[i];
        } else if (damage == bestDamage && damage != 0) {
            // check if better
...
int main(int argc, char* argv[]) {
    ...
    while (!immWin) {
        std::cout << "Checking boost of " << boost << std::endl;
        ...
        while (infection.size() > 0 && immune.size() > 0) {
            ...
            for (int i = 0; i < initID.size(); ++i) {
                if (initID[i].side == 0) {
                    ...
                    kills += attack(immune[initID[i].index], infection[immTargets[initID[i].index]]);
                } else {
                    ...
                    kills += attack(infection[initID[i].index], immune[infTargets[initID[i].index]]);
                }
            }
        ...
        if (immune.size() != 0 && !endless) {
            ...
            std::cout << "Unit HP left " << result << std::endl;
            return 0;
        }
